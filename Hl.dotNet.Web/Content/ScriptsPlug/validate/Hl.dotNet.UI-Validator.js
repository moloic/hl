...
function tipCss(obj, Validatemsg) {
    ...
    if ($('#message').length > 0) {
        ...
        top.TipMsg(Validatemsg, 3000, 'error');
    }
    $(obj).focus();
    ...
    window.setTimeout(docBubbleremove, 3000);
    return false;
}
...
function ChangeCss(obj, Validatemsg) {
    ...
    if ($(obj).attr('class').indexOf('txt') >= 0) {
        if (obj.css('display') == "none") {
            $(obj.siblings()).addClass("tooltipinputerr");
            $(obj.siblings()).removeClass("txt");
        }
        else {
            $(obj).addClass("tooltipinputerr");
            $(obj).removeClass("txt");
        }
    } else if ($(obj).attr('class').indexOf('select') >= 0) {
    ...
    $('#tipTable').show().css("left", $(obj).offset().left > 0 ? $(obj).offset().left - 2 + 'px' : $(obj.siblings()).offset().left - 2 + 'px').css("top", $(obj).offset().top > 0 ? $(obj).offset().top + 25 + 'px' : $(obj.siblings()).offset().top + 25 + 'px');
    ...
    $(obj.siblings().children()).blur(function () {
        $(obj).addClass("txt");
        $(obj.siblings()).removeClass("tooltipinputerr");
        $('#tipTable').remove()
    });
}